...
namespace MJsniffer
{
    ...
    {
        //IP Header fields
        internal byte      byVersionAndHeaderLength;   //Eight bits for version and header length
        internal byte      byDifferentiatedServices;    //Eight bits for differentiated services (TOS)
        internal ushort    usTotalLength;              //Sixteen bits for total length of the datagram (header + message)
        internal ushort    usIdentification;           //Sixteen bits for identification
        internal ushort    usFlagsAndOffset;           //Eight bits for flags and fragmentation offset
        internal byte      byTTL;                      //Eight bits for TTL (Time To Live)
        internal byte      byProtocol;                 //Eight bits for the underlying protocol
        internal short     sChecksum;                  //Sixteen bits containing the checksum of the header
                                                      //(checksum can be negative so taken as short)
        internal uint      uiSourceIPAddress;          //Thirty two bit source IP Address
        internal uint      uiDestinationIPAddress;     //Thirty two bit destination IP Address
        //End IP Header fields
        
        internal byte      byHeaderLength;             //Header length
        internal byte[]    byIPData = new byte[4096];  //Data carried by the datagram
